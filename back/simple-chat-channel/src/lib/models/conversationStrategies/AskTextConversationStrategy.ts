import { TextWebhookMessage } from "../clients/WebhookClient";
...
export class AskTextConversationStrategy extends IConversationStrategy {
  private predefinedResponse: string;

  constructor(input: PostMessageInput) {
    super(input);

    this.predefinedResponse = randomIn(PREDEFINED_RESPONSES);
  }
  ...
  generateWebhookBody(): TextWebhookMessage {
    return {
      type: "text",
